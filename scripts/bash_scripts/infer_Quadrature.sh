...
run_mcmc() {
    ...
    local true_cat="$2"
    local inf_cat="$3"
    local model="DiscreteGammaQuadrature"

    local output_name="outputQuadrature_sim_${true_cat}cats_inf_${inf_cat}cats"
    local log_file="${OUTPUT_DIR}/${output_name}_sim${sim}.out"

    echo "Starting sim ${sim} | true=${true_cat}, inf=${inf_cat}"
    ...
    mpirun -np "$N_CHAINS" rb-mpi \
        --file "${SCRIPT}" \
        --args "sim_${sim}" "sim_Quad_${true_cat}" "${model}" "${output_name}" "${N_CHAINS}" "${inf_cat}" \
        > "${log_file}" 2>&1
}
