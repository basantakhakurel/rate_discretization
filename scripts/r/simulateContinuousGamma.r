...
# This script simulates morphological characters under continuous Gamma-distributed rates.
#
# Usage: Rscript simulateContinuousGamma.r <tree_file> <n_sites> <alpha> <output_dir> <seed>
#
################################################################################
# Reference:
...
if (length(args) < 5) {
  cat("Usage: Rscript simulateContinuousGamma.r <tree_file> <n_sites> <alpha> <output_dir> <seed>\n")
  cat("\nArguments:\n")
  cat("  tree_file   : Path to Newick tree file\n")
  cat("  n_sites     : Number of characters to simulate\n")
  cat("  alpha       : Gamma shape parameter\n")
...
cat("Continuous Gamma Simulation \n")
...
# shape = alpha, rate = alpha ensures mean = 1
site_rates <- rgamma(N_SITES, shape = ALPHA, rate = ALPHA)
...
cladistic_matrix <- build_cladistic_matrix(char_df,
  header = paste("Simulated Dataset using Continuous Gamma -- alpha =", ALPHA, "-- seed =", SEED)
)
