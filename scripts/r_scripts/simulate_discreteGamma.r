...
discrete.gamma.quantile <- function(alpha, k) {
  quantiles <- (1:k) / (k + 1)
  rates <- qgamma(quantiles, shape = alpha, rate = alpha)
  return(rates / mean(rates))
}

gammaCats <- discrete.gamma.quantile(alpha = H, k = NUM_CATEGORIES)
