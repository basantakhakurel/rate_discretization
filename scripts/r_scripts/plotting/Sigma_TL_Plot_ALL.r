#!/usr/bin/env Rscript
# R script to plot the posteriors for Lognormal output
...
CONFIG <- list(
  heterogeneity_levels = list(
    `1-Order` = list(
      ...
      true_sigma = 0.587405,
      sigma_limits = c(0.3, 1.0),
      tl_limits = c(0.45, 1.5)
    ),
    `2-Order` = list(
      label = "2-Orders",
      path = "cluster_output/two_orders_magnitude/inference_results",
      true_sigma = 1.1748,
      sigma_limits = c(0.6, 2.0),
      tl_limits = c(0.45, 1.5)
    ),
    `3-Order` = list(
      label = "3-Orders",
  ...
  plot_settings = list(
    ...
    output_height = 20,
    dpi = 450,
    sigma_line_color = "#e74c3c",
    tl_line_color = "#8e44ad",
    tag_size = 30
  )
)

# function to load and parse output with Lognormal
load_and_parse_data <- function(data_path, level_label, true_sigma, rep_num = 1) {
  ...
  if (length(target_dirs) == 0) {
    warning("No Lognormal model result directories found in: ", data_path)
    return(data.frame())
  }
  ...
  if (nrow(combined_data) > 0) {
    combined_data$true_sigma <- true_sigma
...
create_sigma_plot <- function(data, x_lab, plot_title, limits_list) {
  p <- build_ridge_panel(
    data = data,
    x_var = "sigma",
    x_lab = x_lab,
  ...
  p <- p + geom_vline(
    aes(xintercept = .data[["true_sigma"]]),
    linetype = "dashed",
    color = CONFIG$plot_settings$sigma_line_color,
    linewidth = 0.9
  )
...
save_plot_combo <- function(p_top, p_bottom, file_suffix, plot_output_dir, rep_num) {
  ...
  # Save the plot
  filename <- file.path(plot_output_dir, paste0("Lognormal_", file_suffix, "rep_", rep_num, ".pdf"))
  ggsave(
    filename,
    plot_combo,
...
main <- function(rep_num = 1, plot_output_dir = "plots") {
  ...
  sigma_limits_list <- sapply(CONFIG$heterogeneity_levels, `[[`, "sigma_limits", simplify = FALSE)
  tl_limits_list <- sapply(CONFIG$heterogeneity_levels, `[[`, "tl_limits", simplify = FALSE)

  names(sigma_limits_list) <- level_labels
  names(tl_limits_list) <- level_labels

  data_A <- all_data %>%
    filter((sim_model == "discreteLognormalMedian" | sim_model == "continuousLognormal") & inf_model == "discreteLognormalMedian")
  title_A <- "Simulation and Inference with Lognormal Distribution"

  sigma_xlab <- expression("Shape of Lognormal (" * sigma * ")")
  tl_xlab <- "Tree Length"

  pA_Sigma <- create_sigma_plot(
    data = data_A,
    x_lab = sigma_xlab,
    plot_title = NULL,
    limits_list = sigma_limits_list
  )

  pB_TL <- create_tl_plot(
    data = data_A,
    x_lab = tl_xlab,
    plot_title = NULL,
    limits_list = tl_limits_list
  )

  save_plot_combo(
    p_top = pA_Sigma,
    p_bottom = pB_TL,
    file_suffix = "Sigma_TL_",
...
cat("\n Starting Combined Lognormal Plot Generation for Replicate", rep_to_process, "...\n")
